...
## 安装
> composer require cuicmf/think-auth:dev-master

## 配置
在config目录中新建`auth.php`,内容为：
```
// auth配置
'auth'  => [
    'auth_on'           => 1, // 权限开关
    ...
    'auth_group_access' => 'auth_group_access', // 用户组关系不带前缀表名
...
// 检测权限
if($auth->check('rule',1)){// 第一个参数是规则名称,第二个参数是用户UID
	//有显示操作的权限
}else{
	//没有显示操作的权限
}
...
按照用户积分进行判断， 假设我们的用户表 (think_user) 有字段 score 记录了用户积分。 
...
这里 `{score}` 表示 `think_user` 表 中字段 `score` 的值。 
